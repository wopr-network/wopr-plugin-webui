...
describe("api.pollPluginEndpoint", () => {
  it("should GET /api/plugins/:name/proxy/:endpoint", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/api/plugins/%40wopr-network%2Fplugin-discord/proxy/healthz",
      { headers: { "Content-Type": "application/json" } },
    );
  ...
  it("should throw on non-ok response", async () => {
    ...
    await expect(
      api.pollPluginEndpoint("missing-plugin", "/healthz"),
...
describe("api.getPluginConfig", () => {
  it("should GET /api/plugins/:name/config", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/api/plugins/%40wopr-network%2Fplugin-discord/config",
      { headers: { "Content-Type": "application/json" } },
    );
...
describe("api.setPluginConfigValue", () => {
  it("should PUT /api/plugins/:name/config/:key with value", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/api/plugins/%40wopr-network%2Fplugin-discord/config/guildId",
      {
        method: "PUT",
      ...
      manifests: [
        {
          ...
          webui: {
            ...
            routes: [
              { path: "/plugins/test/dashboard", title: "Dashboard", component: "/dashboard.js" },
            ],
          },
        },
      ],
      ...
      manifests: [
        {
          ...
          configSchema: {
            ...
            fields: [
              ...
              { name: "region", type: "select", label: "Region", options: [{ value: "us", label: "US" }, { value: "eu", label: "EU" }] },
              { name: "debug", type: "boolean", label: "Debug Mode" },
            ],
          },
          webui: {
            panels: [
              { id: "cfg", title: "Settings", type: "config", configFields: ["apiKey", "region"] },
            ],
          },
        },
      ],
