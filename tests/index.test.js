...
describe("webui plugin init", () => {
  ...
  it("should log error and return early if dist/ does not exist", async () => {
    ...
    expect(ctx.log.error).toHaveBeenCalledWith(
      expect.stringContaining("dist/ folder not found"),
    );
  ...
  it("should not call registerWebUiExtension when it is not available", async () => {
    const ctx = createMockContext({ registerWebUiExtension: undefined });
    fsMock.existsSync.mockReturnValue(true);
    ...
    expect(ctx.log.info).toHaveBeenCalledWith(
      expect.stringContaining("Web UI available"),
    );
  });
...
describe("webui plugin shutdown", () => {
  ...
  it("should reject if server.close errors", async () => {
    const ctx = createMockContext();
    fsMock.existsSync.mockReturnValue(true);
    ...
    await expect(plugin.shutdown()).rejects.toThrow("close error");
  });
});
